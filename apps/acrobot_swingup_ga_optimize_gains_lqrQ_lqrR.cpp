...
int main(int argc, char **argv)
{
    ...
    */
    ...
    simulator.set_monitor([&plant, &controller, &exp_cfg, &t_in_eq, &current_time, &last_time](const drake::systems::Context<double>& context) {
        ...
        double cost{error.dot(S*error)}; // S is influenced by the chromosome! Not good.
        
        if (cost < exp_cfg.eqErrorTol) { // when it goes in and out it wont work
    ...
    std::vector<double> decoded(exp_cfg.nVariables);
    Eigen::Matrix4d Q = Eigen::Matrix4d::Identity();
    Eigen::Matrix<double,1,1> R{1.0};
    Eigen::Matrix4d A = controller->get_A();
    Eigen::Matrix<double,4,1> B = controller->get_B();
    ...
    for(size_t i = 0; i < exp_cfg.runs; i++) {
        ...
        for(auto& individual : population.individuals) {
            ...
            decoded = individual.DecodeGrayChromosome(exp_cfg.maxGain);
            for(size_t j = 0; j < 3; j++) {
                ctrl_cfg.controllerGains[j] = decoded[j] + exp_cfg.maxGain;
            }
                      
            for(size_t j = 0; j < 4; j++) {
                Q(j,j) = decoded[3] + exp_cfg.maxGain + 0.1; // must be larger than 0
            }            
            R(0,0) = decoded[4] + exp_cfg.maxGain + 0.1; // must be larger than 0
            
            // need to redo the lqr -> new K_ and S_ matricies if Q and R are part
            // of the chromosome.
            // fast if the linearization is stored.
            // perhaps good to separate from this
