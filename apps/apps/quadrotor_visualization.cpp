...
//using namespace underact_robotics::controllers::cartpole;
/*
...
int main()
{
    ...
    auto drone_ind = parser.AddModels("../models/quadrotor/skydio_x2.urdf");
    auto table_ind = parser.AddModels("../models/table_top.urdf");
    plant.Finalize();
    ...
    YAML::Node config = YAML::LoadFile("../configs/quadtest.yaml");

    Eigen::Vector<double, 13> x0;
    std::vector<double> v = config["init"].as<std::vector<double>>(); 
    for(size_t i = 0; i < 13; i++) {
        x0(i) = v[i];
    }

    int a = plant.num_positions(drone_ind[0]);
    int b = plant.num_velocities(drone_ind[0]);

    std::cout << "positions: " <<  a << std::endl;
    std::cout << "velocities: " << b << std::endl;

    plant.SetPositionsAndVelocities(&plant_context, drone_ind[0], x0);
