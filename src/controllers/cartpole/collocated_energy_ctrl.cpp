...
namespace modelling_control_and_simulation::controllers::cartpole {
    ...
    CollocatedEnergyCtrl::CollocatedEnergyCtrl(const CollocatedEnergyCtrlConfig& cfg) : config_(cfg) 
    {
        ...
        DeclareVectorOutputPort("output", 1, &CollocatedEnergyCtrl::CalcControlForce);
    }
    ...
    void CollocatedEnergyCtrl::SetUpPlantAndLqr() {
        ...
        b1_ = plant_->GetJointByName("x").default_damping_vector()[0];
        b2_ = plant_->GetJointByName("theta").default_damping_vector()[0];
    ...
    void CollocatedEnergyCtrl::CalcControlForce(const drake::systems::Context<double>& context,
                         drake::systems::BasicVector<double>* output) const
    {
        ...
        double u =  ComputeForce(tmp);
        output->SetAtIndex(0, u);
    }


    double CollocatedEnergyCtrl::ComputeForce(const Eigen::Vector4d& state) const
{

    //note we, do not care about the "error" in x, although it would be nice to have it at zero
    // but lqr shouldnt care.
    Eigen::Vector4d error = state-eq_;
    
    error(0) = 0.0; // but this can cause drift, restore after cost calc
    ...
    if (cost < config_.cost) {
       ...
       u = (-K_*error)(0);
        ...
        double sat_energy = std::clamp(config_.controllerGains[2]*std::cos(state(1))*(E - E_eq_)*state(3),
                                        -config_.saturation,
                                         config_.saturation);
    ...
    if(config_.jointTorqueLimits.has_value()) {
        u = std::clamp(u, -config_.jointTorqueLimits.value().at(0),
                           config_.jointTorqueLimits.value().at(0)); // force limit here.
    }
